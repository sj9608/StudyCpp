...
위 예제에서 BBB 클래스의 생성자를 확인해보면 다음과 같이 선언하였다.

``` C++
BBB(const AAA& ref) : mem(ref) { }
```

이니셜라이저를 이용하면 선언과 도잇에 초기화가 이뤄지는 형태로 바이너리 코드가 생성된다.

즉, 위의 이니셜라이저를 통한 초기화 구문은 다음 문장에 비유할 수 있다.

``` C++
AAA mem = ref;
```

그래서 복사 생성자만 호출이 된 것이다.

반면 CCC 클래스에서는 생성자의 몸체 부분에서 대입연산을 통해 초기화를 하였기 때문에 선언과 초기화를 각각 별도로 진행하는 형태로 바이너리 코드가 생성된다.

이렇듯 이니셜 라이저를 통해 초기화를 진행하면, 함수의 호출 횟수를 줄일 수 있어서. 그리고 초기화 과정을 단순화 시킬 수 있어서 약간의 성능향상을 기대할 수 있다.